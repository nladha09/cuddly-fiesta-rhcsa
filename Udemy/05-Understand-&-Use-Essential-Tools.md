...
Commands:
- `more /rhcsa.txt | grep -v -i "sEd" > /root/results.txt` - to copy line not containing "sEd" or "SeD". `-v` = takes input line by line, and outputs only the lines in which "word" does not appear
